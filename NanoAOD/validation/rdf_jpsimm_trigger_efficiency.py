...
trigger for Jpsi->mumu final state in MC and Data and calculates
trigger efficiency corrections for B->mm decays and their ratios.
...
# process_only_eras = ["Run3Summer22", "Run3Summer22EE"]
process_only_eras = None
...
# sample_names = ["BsToJpsiPhi", "BuToJpsiK", 'ZeroBias', 'EGammaLowPt',
#                'EGamma', 'ParkingDoubleElectronLowMass']
...
# output_path = "/eos/home-d/dmytro/www/plots/tmp/fsfu-trigger_efficiency/"
...
class DataProcessor:
            ...
            for era, n_versions in self.eras[run].items():
                ...
                for version in range(1, n_versions + 1):
                    ...
                    for pd in egamma_pds:
                        ...
                        if re.search("Run2024", era):
                            self.samples["EGammaLowPt"]["files"][era].append(
                                path4 + f"/{pd}+{era}-PromptReco-v{version}+MINIAOD/*root",
                            )
    ...
    def process_sample(self, sample_name, era):
        if process_only_eras != None and era not in process_only_eras:
            return
        ...
        if sample_name == "BsToJpsiPhi":
            ...
            rdf_nom_bs = rdf.Filter("Sum(nom_bs_cands)>0")
            rdf_nom_bs = rdf_nom_bs.Define("jpsi_pt",     "bkkmm_mm_kin_pt[nom_bs_cands]")
            rdf_nom_bs = rdf_nom_bs.Define("jpsi_eta",    "bkkmm_mm_kin_eta[nom_bs_cands]")
            rdf_nom_bs = rdf_nom_bs.Define("jpsi_m1_pt",  "bkkmm_mu1_pt[nom_bs_cands]")
            rdf_nom_bs = rdf_nom_bs.Define("jpsi_m1_eta", "bkkmm_mu1_eta[nom_bs_cands]")
            rdf_nom_bs = rdf_nom_bs.Define("jpsi_m2_pt",  "bkkmm_mu2_pt[nom_bs_cands]")
            rdf_nom_bs = rdf_nom_bs.Define("jpsi_m2_eta", "bkkmm_mu2_eta[nom_bs_cands]")
            rdf_nom_bs = rdf_nom_bs.Define("jpsi_dphi",   "bkkmm_mm_dphi[nom_bs_cands]")
            rdf_nom_bs = rdf_nom_bs.Define("b_pt",        "bkkmm_jpsikk_pt[nom_bs_cands]")
            rdf_nom_bs = rdf_nom_bs.Define("b_eta",       "bkkmm_jpsikk_eta[nom_bs_cands]")
        
            self.book_histo_2D(rdf_histos, rdf_nom_bs, sample_name, era, "Bs_All")
            self.book_histo_2D(rdf_histos, rdf_nom_bs.Filter("HLT_DoubleMu4_3_LowMass"),
                               sample_name, era, "Bs_HLT_DoubleMu4_3_LowMass")

            # self.book_histo_3D(rdf_histos, rdf_nom_bs, sample_name, era, "Bs_All3D")
            self.book_histo_ND(rdf_histos, rdf_nom_bs, sample_name, era, "Bs_All3D",  bins,    vars)
            self.book_histo_ND(rdf_histos, rdf_nom_bs, sample_name, era, "Bs_All4D",  bins4D,  vars4D)
            self.book_histo_ND(rdf_histos, rdf_nom_bs, sample_name, era, "Bs_All4D2", bins4D2, vars4D2)
            self.book_histo_ND(rdf_histos, rdf_nom_bs, sample_name, era, "Bs_All4D3", bins4D3, vars4D3)
            self.book_histo_ND(rdf_histos, rdf_nom_bs, sample_name, era, "Bs_All4D4", bins4D4, vars4D4)
            self.book_histo_ND(rdf_histos, rdf_nom_bs, sample_name, era, "Bs_All5D",  bins5D,  vars5D)

            self.book_histo_ND(rdf_histos, rdf_nom_bs, sample_name, era, "Bs",  b_bins,    b_vars)
            ...
            if era == "RunIII2024Summer24":
                ...
                self.book_histo(rdf_histos, rdf_lowpt_bs, sample_name, era, "Bs_AllLowPt",
                                low_pt_bins, low_pt_vars)
                self.book_histo(rdf_histos, rdf_lowpt_bs.Filter("HLT_DoubleMu2_Jpsi_LowPt"),
                                sample_name, era, "Bs_HLT_DoubleMu2_Jpsi_LowPt",
                                low_pt_bins, low_pt_vars)
        ...
        if run == "Run2022":
            ...
            l1_selection_unprescaled = "L1_DoubleMu0er1p4_SQ_OS_dR_Max1p4||L1_DoubleMu4er2p0_SQ_OS_dR_Max1p6" + \
                "||L1_DoubleMu4p5_SQ_OS_dR_Max1p2"
            # l1_selection_real_mix = f"{l1_selection_unprescaled}||L1_DoubleMu0er2p0_SQ_OS_dEta_Max1p5||L1_DoubleMu4_SQ_OS_dR_Max1p2"
...
if __name__ == "__main__":
    ...
    if perform_method_validation:
        ...
        h_target = processor.histos['BsToJpsiPhi'][campaign]['Bs_All']
        h_target3D = processor.histos['BsToJpsiPhi'][campaign]['Bs_All3D']

        compute_efficiency(processor.histos, "BsToJpsiPhi", campaign,
                           "Bs_HLT_DoubleMu4_3_LowMass", "Bs_All", "\tBsToJpsiPhi")
        ...
        for pd in pds:
            for run in ["Run2022", "Run2023", "Run2024"]:
    ...
    if compute_run_average_efficiencies:
        ...
        pd = 'EGamma'
        for run,campaing in [("Run2022", 'Run3Summer22'),
                             ("Run2023", 'Run3Summer23'),
                             ("Run2024", 'RunIII2024Summer24')]:
            h_target = processor.histos['BsToJpsiPhi'][campaing]['Bs_All']
            compute_reweighted_efficiency(processor.histos, pd, run,
                                          "HLT_DoubleMu4_3_LowMass", "All", h_target)
        ...
        pd = 'EGammaLowPt'
        h_target = processor.histos['BsToJpsiPhi']['RunIII2024Summer24']['Bs_AllLowPt']
        compute_efficiency(processor.histos, "BsToJpsiPhi", "RunIII2024Summer24",
                           "Bs_HLT_DoubleMu2_Jpsi_LowPt", "Bs_AllLowPt", "BsToJpsiPhi")
    ...
    if make_plots:
        for sample_name in processor.histos:
            ...
            for run in ["Run2022", "Run2023", "Run2024", "Run3Summer22",
    ...
    if compute_corrections:
        for run, campaign in [ ("Run2022", "Run3Summer22"), ("Run2023", "Run3Summer23"), ("Run2024", "RunIII2024Summer24")]:

            h4_bu = processor.histos['BuToJpsiK'][campaign]['Bu']
            h4_bs = processor.histos['BsToJpsiPhi'][campaign]['Bs']
            ...
            for b_pt_bin in range(len(b_pt_bins) - 1):
                ...
                results.append((
                    (b_pt_bins[b_pt_bin]+b_pt_bins[b_pt_bin+1])/2, corr,
                    (b_pt_bins[b_pt_bin+1]-b_pt_bins[b_pt_bin])/2, corr_err))
                results_for_analysis.append(((b_pt_bins[b_pt_bin], b_pt_bins[b_pt_bin+1]), (corr, corr_err)))
            ...
            for i in range(n):
                gr.SetPoint(i, results[i][0], results[i][1])
                gr.SetPointError(i, results[i][2], results[i][3])
            ...
            print_canvas(f"rs_correction_nom-{run}", output_path)
            with open(f"{output_path}/rs_correction_nom-{run}.json", "w") as f:
