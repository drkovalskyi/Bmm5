...
// Timing instrumentation. Gated at compile time by BMM_PROFILING: without
// the flag, PerfClock::now() returns an empty struct and perfTimer is a
// no-op, so all perfTimer(...) call sites collapse to nothing under the
// optimizer.
#ifdef BMM_PROFILING
...
inline void perfTimer(std::atomic<long long>&, PerfClock::time_point) {}
#endif
...
private:
  ...
  float trackIPSwrtBS(const reco::Track& track);
  
  std::unique_ptr<FitCandidate>
  buildKsCandidate(const edm::Event& iEvent,
		   const pat::PackedCandidate& had1,
		   const pat::PackedCandidate& had2,
		   KsSelectionType type);
  
  const pat::CompositeCandidate*
  buildAndDressKsCandidate(pat::CompositeCandidateCollection& hh_collection,
  ...
  KinematicFitResult 
  vertexCandsWithKinematicFitter(const pat::PackedCandidate& pfCand1,
  ...
  KinematicFitResult 
  vertexWithKinematicFitter(const bmm::Candidate& lepton,
			    const reco::Track* track,
			    double mass = PionMass_);
  ...
  KinematicFitResult
  fitBToLLhh( const bmm::Candidate& lepton1,
	      const bmm::Candidate& lepton2,
  ...
  KinematicFitResult
  fitBToJpsiKs(const bmm::Candidate& lepton1,
	       const bmm::Candidate& lepton2,
	       const std::unique_ptr<FitCandidate>& ksCand,
	       bool jpsi_mass_constraint=false,
	       bool ks_mass_constraint=false);
  ...
  fillBtoKllInfo(pat::CompositeCandidate& bCand,
		 ...
		 const pat::PackedCandidate & kaon); 
  void
  fillBtoLLhhInfo(pat::CompositeCandidate& bCand,
		  const edm::Event& iEvent,
		  const bmm::Candidate& muon1,
		  const bmm::Candidate& muon2,
		  const pat::PackedCandidate & kaon1,
  ...
  double minKsMass_;
  double maxKsMass_;
  double minKsMassVeryLoose_;
  double maxKsMassVeryLoose_;
...
DileptonPlusXProducer::DileptonPlusXProducer(const edm::ParameterSet &iConfig):
  ...
  ptMinHad_(       iConfig.getParameter<double>( "HadronMinPt" ) ),
  etaMaxHad_(      iConfig.getParameter<double>( "HadronMaxEta" ) ),
  ptMinEl_(         iConfig.getParameter<double>( "ElectronMinPt" ) ),
  etaMaxEl_(        iConfig.getParameter<double>( "ElectronMaxEta" ) ),
  DCASigMinKaon_(   iConfig.getParameter<double>( "HadronMinDCASig" ) ),
  ...
  minKsMassVeryLoose_( iConfig.getParameter<double>( "minKsMassVeryLoose" ) ),
  maxKsMassVeryLoose_( iConfig.getParameter<double>( "maxKsMassVeryLoose" ) ),
...
DileptonPlusXProducer::buildTnPCandidate(const pat::Muon& muon1,
  ...
  if (cand.mu1_probe && cand.mu2_tag) {
    ...
    if (cand.jpsi_mass1 > min_mass && cand.jpsi_mass1 < max_mass && probe->charge() != muon2.charge())
  ...
  if (cand.mu2_probe && cand.mu1_tag) {
    ...
    if (cand.jpsi_mass2 > min_mass && cand.jpsi_mass2 < max_mass && probe->charge() != muon1.charge())
...
float DileptonPlusXProducer::trackIPSwrtBS(const reco::Track& track) {
  if (track.dxyError(*beamSpot_) > 0)
    return std::abs(track.dxy(*beamSpot_)) / track.dxyError(*beamSpot_);
  else
    return -1.0;
}
...
void DileptonPlusXProducer::fillBtoLLhhInfo(pat::CompositeCandidate& bCand,
					    const edm::Event& iEvent,
					    const bmm::Candidate& lepton1,
					    const bmm::Candidate& lepton2,
					    const pat::PackedCandidate & kaon1,
					    ...
					    const std::unique_ptr<FitCandidate>& ksCand)
{
  ...
  bCand.addUserFloat("kaon1_sdxy_bs", 
		     kaon1.bestTrack()->dxyError()>0 ?
		     fabs(kaon1.bestTrack()->dxy(*beamSpot_))/kaon1.bestTrack()->dxyError():0.0);
  ...
  bCand.addUserFloat("kaon2_sdxy_bs", 
		     kaon2.bestTrack()->dxyError()>0 ?
		     fabs(kaon2.bestTrack()->dxy(*beamSpot_))/kaon2.bestTrack()->dxyError():0.0);
  bCand.addUserInt("kaon2_charge", kaon2.charge());
  
  bCand.addUserFloat("kk_mass",    (kaon1.p4() + kaon2.p4()).mass());
  bCand.addUserFloat("kpi_mass",   (kaon1.p4() + pion2.p4()).mass());
  bCand.addUserFloat("pik_mass",   (pion1.p4() + kaon2.p4()).mass());
  bCand.addUserFloat("pipi_mass",  (pion1.p4() + pion2.p4()).mass());
  ...
  if (bToKKll.vtxProb() > 0.001 && fabs((lepton1.p4() + lepton2.p4()).mass() - JPsiMass_) < 0.2) { 
    ...
    perfTimer(t_fit_jpsikk_, t0_jkk);
  }
  ...
  perfTimer(t_displacement_, t0_d); }
					 ...
					 const reco::Track* refTrack2)
{
  ...
  for (const auto& pfCand: *pfCandHandle_.product()) {
    ...
    ////// Pre-computing needed information

    bool save_track = false;
    unsigned int track_index = tracks.size();
    ...
    if (track_index == tracks.size()) tracks.push_back(track);
    ...
    //// PV + track
    // - compute just impact parameter and its significance
    // - no need to refit, since PV has smaller uncertainty
    //   and wont change much
...
void
DileptonPlusXProducer::fillTrackInfo(pat::CompositeCandidateCollection& output_collection,
				     const std::vector<const reco::Track*>& tracks,
				     const edm::Event& iEvent)
{
  for (auto track: tracks){
    pat::CompositeCandidate tk_cand;
    tk_cand.addUserFloat("pt", track->pt());
    tk_cand.addUserFloat("eta", track->eta());
    tk_cand.addUserFloat("phi", track->phi());
    tk_cand.addUserInt("charge", track->charge());
    tk_cand.addUserFloat("vtx_x", track->vx());
    tk_cand.addUserFloat("vtx_y", track->vy());
    tk_cand.addUserFloat("vtx_z", track->vz());

    tk_cand.addUserFloat("trkValidFrac",  track->validFraction());
    tk_cand.addUserFloat("trkNormChi2",   track->normalizedChi2());
	
    tk_cand.addUserInt("pixelPattern",    bmm::get_pixel_pattern(track->hitPattern()));
    tk_cand.addUserInt("nPixels",         track->hitPattern().numberOfValidPixelHits());
    tk_cand.addUserInt("nValidHits",      track->hitPattern().numberOfValidTrackerHits());
    tk_cand.addUserInt("nLostHitsInner",  track->hitPattern().numberOfLostTrackerHits(reco::HitPattern::MISSING_INNER_HITS));
    tk_cand.addUserInt("nLostHitsOn",     track->hitPattern().numberOfLostTrackerHits(reco::HitPattern::TRACK_HITS));
    tk_cand.addUserInt("nLostHitsOuter",  track->hitPattern().numberOfLostTrackerHits(reco::HitPattern::MISSING_OUTER_HITS));
	
    tk_cand.addUserInt("trkLayers",           track->hitPattern().trackerLayersWithMeasurement());
    tk_cand.addUserInt("trkLostLayersInner",  track->hitPattern().trackerLayersWithoutMeasurement(reco::HitPattern::MISSING_INNER_HITS));
    ...
    // Gen Information
    if (isMC_) {
      int gen_pdgId(0), gen_mpdgId(0);
      for (auto const & genParticle: *packedGenParticles_) {
	if (dr_match(*track, genParticle.p4())) {
	  gen_pdgId = genParticle.pdgId();
	  if (genParticle.mother(0))
	    gen_mpdgId = genParticle.mother(0)->pdgId();
	  break;
	}
      }
      tk_cand.addUserInt("gen_pdgId", gen_pdgId);
      tk_cand.addUserInt("gen_mpdgId", gen_mpdgId);
    }    
    output_collection.push_back(tk_cand);
  }
}
...
DileptonPlusXProducer::injectBhhHadrons(std::vector<bmm::Candidate>& good_lepton_candidates){
  ...
  for (const auto& pfCand: *pfCandHandle_.product()){
    ...
    if (fabs(pfCand.eta()) > maxBhhTrkEta_) continue;
					  ...
					  const bmm::Candidate& lepton2)
{
  ...
  for (unsigned int k = 0; k < nPFCands; ++k) {
    ...
    // pion hypothesis
    pat::PackedCandidate pionCand1((*pfCandHandle_)[k]);
    pionCand1.setMass(PionMass_);
    ...
    bool good3ProngVtx = true;

    if (maxTwoTrackDOCA_ > 0 and l1_kaon_doca > maxTwoTrackDOCA_) good3ProngVtx = false;
    if (maxTwoTrackDOCA_ > 0 and l2_kaon_doca > maxTwoTrackDOCA_) good3ProngVtx = false;
    ...
    if (good3ProngVtx and goodBtoLLK){
      ...
      perfTimer(t_fillBtoKllInfo_, t0); }
    ...
    for (unsigned int k2 = 0; k2 < nPFCands; ++k2) {
      ...
      double l2_kaon2_doca = distanceOfClosestApproach(lepton2.track(),
						       kaonCand2.bestTrack());
      bool good4ProngVtx = good3ProngVtx;
      if (good4ProngVtx) {
	if (maxTwoTrackDOCA_>0 and l1_kaon2_doca > maxTwoTrackDOCA_) good4ProngVtx = false;
	if (maxTwoTrackDOCA_>0 and l2_kaon2_doca > maxTwoTrackDOCA_) good4ProngVtx = false;
      ...
      perfTimer(t_buildKsCandidate_, t0_ks);
      
      // make sure that all hypothese fit within the B mass window to avoid biasing the selection
      bool goodBtoLLhh = false;
      bool goodBtoLLKs = false;
      double llkk_mass   = (lepton1.p4() + lepton2.p4() + kaonCand1.p4() + kaonCand2.p4()).mass();
      double llpik_mass  = (lepton1.p4() + lepton2.p4() + pionCand1.p4() + kaonCand2.p4()).mass();
      double llkpi_mass  = (lepton1.p4() + lepton2.p4() + kaonCand1.p4() + pionCand2.p4()).mass();
      double llpipi_mass = (lepton1.p4() + lepton2.p4() + pionCand1.p4() + pionCand2.p4()).mass();
      if ( (llkk_mass   >= minBhhllMass_ && llkk_mass   <= maxBhhllMass_ ) ||
	   (llpik_mass  >= minBhhllMass_ && llpik_mass  <= maxBhhllMass_ ) ||
	   (llkpi_mass  >= minBhhllMass_ && llkpi_mass  <= maxBhhllMass_ ) ||
	...
	perfTimer(t_fillBtoLLhhInfo_, t0); }
	// FIXME
...
std::unique_ptr<FitCandidate>
DileptonPlusXProducer::buildKsCandidate(const edm::Event& iEvent,
					const pat::PackedCandidate& had1,
					const pat::PackedCandidate& had2,
					KsSelectionType type) {
  ...
  if (ks_mass < 1.0){
    ...
    if (preprocess(ksCand, iEvent, pion1, pion2)){
      ...
      bool ks_loose = abs(had1.pdgId()) == 211 && had1.pdgId() == -had2.pdgId() &&
	vtxFit.mass() > minKsMass_ && vtxFit.mass() < maxKsMass_;
      bool ks_sideband = fabs(vtxFit.lxy()) > 1.0 && vtxFit.mass() > minKsMassVeryLoose_ &&
	vtxFit.mass() < maxKsMassVeryLoose_;
...
const pat::CompositeCandidate*
DileptonPlusXProducer::buildAndDressKsCandidate(pat::CompositeCandidateCollection& hh_collection,
						pat::CompositeCandidateCollection& iso_collection,
						std::vector<const reco::Track*>& tracks,
						const edm::Event& iEvent,
						const pat::PackedCandidate& had1,
						const pat::PackedCandidate& had2,
						KsSelectionType type) {
  auto fitCand = buildKsCandidate(iEvent, had1, had2, type);
  if (fitCand){
    fillIsolationInfo(fitCand->first, fitCand->second, -1, hh_collection.size(),
		      iso_collection, tracks, iEvent, had1.bestTrack(), had2.bestTrack());
    hh_collection.push_back(std::move(fitCand->first));
...
void DileptonPlusXProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if ( good_muon_candidates.size() > 1 ){
	...
	perfTimer(t_fillDileptonInfo_, t0_fdi);
  ...
  if (n_loose_muon_candidates > 1) {
    for (unsigned int i = 0; i < n_loose_muon_candidates; ++i) {
      const bmm::Candidate & cand1 = loose_muon_candidates.at(i).first;
      for (unsigned int j = 0; j < n_loose_muon_candidates; ++j) {
	const bmm::Candidate & cand2 = loose_muon_candidates.at(j).first;
	if (i==j) continue;

	// Enforce momentum ordering
	if (cand2.pt() > cand1.pt()) continue;
	const pat::Muon& muon1 = muonHandle->at(cand1.index());
	const pat::Muon& muon2 = muonHandle->at(cand2.index());
	
	auto tnp_cand = buildTnPCandidate(muon1, muon2);
	if (not tnp_cand.jpsi_probe1 && not tnp_cand.jpsi_probe2) continue;
	
	pat::CompositeCandidate dimuonCand(std::string("tnp"));
	dimuonCand.addDaughter( cand1 , "muon1");
	dimuonCand.addDaughter( cand2 , "muon2");
	addP4.set( dimuonCand );
	
	fill_basic_information(dimuonCand, iEvent, cand1, cand2);
	...
	dimuonCand.addUserFloat("mu1_sta_pt", tnp_cand.mu1_probe ? muon1.standAloneMuon()->pt() : 0);
	dimuonCand.addUserFloat("mu2_sta_pt", tnp_cand.mu2_probe ? muon2.standAloneMuon()->pt() : 0);

	// Kinematic fits
	KinematicFitResult fit_tt;
	if (tnp_cand.mu1_tag && tnp_cand.mu2_tag) {
	  fit_tt = vertexWithKinematicFitter(
	    std::vector<const reco::Track*>{
	      muon1.innerTrack().get(),
	      muon2.innerTrack().get()
	    },
	    std::vector<float>{MuonMass_, MuonMass_});
	}
	dimuonCand.addUserInt(   "tag1_tag2_kin_valid",   fit_tt.valid() );
	dimuonCand.addUserFloat( "tag1_tag2_kin_prob",    fit_tt.vtxProb() );
	dimuonCand.addUserFloat( "tag1_tag2_kin_mass",    fit_tt.mass() );
	dimuonCand.addUserFloat( "tag1_tag2_kin_massErr", fit_tt.massErr() );

	KinematicFitResult fit_probe1;
	if (tnp_cand.jpsi_probe1) {
	  fit_probe1 = vertexWithKinematicFitter(
	    std::vector<const reco::Track*>{
	      muon1.outerTrack().get(),
	      muon2.innerTrack().get()
	    },
	    std::vector<float>{MuonMass_, MuonMass_});
	}
	dimuonCand.addUserInt(   "probe1_tag2_kin_valid",       fit_probe1.valid() );
	dimuonCand.addUserFloat( "probe1_tag2_kin_prob",    fit_probe1.vtxProb() );
	dimuonCand.addUserFloat( "probe1_tag2_kin_mass",    fit_probe1.mass() );
	dimuonCand.addUserFloat( "probe1_tag2_kin_massErr", fit_probe1.massErr() );

	KinematicFitResult fit_probe2;
	if (tnp_cand.jpsi_probe2) {
	  fit_probe2 = vertexWithKinematicFitter(
	    std::vector<const reco::Track*>{
	      muon1.innerTrack().get(),
	      muon2.outerTrack().get()
	    },
	    std::vector<float>{MuonMass_, MuonMass_});
	}
	dimuonCand.addUserInt(   "probe2_tag1_kin_valid",       fit_probe2.valid() );
	dimuonCand.addUserFloat( "probe2_tag1_kin_prob",    fit_probe2.vtxProb() );
	dimuonCand.addUserFloat( "probe2_tag1_kin_mass",    fit_probe2.mass() );
	dimuonCand.addUserFloat( "probe2_tag1_kin_massErr", fit_probe2.massErr() );
  ...
  if (nPFCands > 1 && (recoDstar_ || recoD0pipi_ || recoD0Kpi_ || recoKspipi_ || recoKstar_)){
	...
	const auto* ksCand = buildAndDressKsCandidate(*hh_collection, *iso_collection, interestingTracks,
						      iEvent, cand1, cand2, KsmmSelection);
...
KinematicFitResult 
DileptonPlusXProducer::vertexCandsWithKinematicFitter(const pat::PackedCandidate& pfCand1,
						      const pat::PackedCandidate& pfCand2)
{
  std::vector<const reco::Track*> trks;
  std::vector<float> masses;
...
KinematicFitResult 
DileptonPlusXProducer::vertexWithKinematicFitter(const bmm::Candidate& lepton,
						 const reco::Track* track,
						 double mass)
{
  std::vector<const reco::Track*> trks;
  std::vector<float> masses;
  trks.push_back( lepton.track() );
  masses.push_back( lepton.mass() );
    ...
    const bmm::Candidate& muon2)
{
  ...
  for (size_t iks = 0; iks < ksVerticesHandle->size(); ++iks) {
    ...
    if (minBhhllVtxProb_ > 0 && bCand.userFloat("kin_vtx_prob") < minBhhllVtxProb_) continue;
					   ...
					   const std::string& ks_prefix)
{
  ...
  if (ksCand and fabs((lepton1.p4() + lepton2.p4()).mass() - JPsiMass_) < 0.2) {
    ...
    perfTimer(t_fit_jpsiks_, t0_jks);
  }
				 ...
				 float mass_constraint)
{
  ...
  // Pre-fit: dimuon composite + kaon as a 2-particle vertex fit. Only
  // if this easier fit converges with vtxProb > 0.001 do we proceed
  // to the direct 3-track ultimate fit. Ill-conditioned
  // configurations that destabilise the 3-track fit are rejected
  // here.
  {
    auto llFit = vertexLeptonsWithKinematicFitter(lepton1, lepton2);
    if (!llFit.valid()) return result;
    auto llTree = llFit.tree();

    if (mass_constraint > 0.f) {
      ParticleMass pMass = mass_constraint;
      MassKinematicConstraint mc(pMass, JPsiMassErr_);
      KinematicParticleFitter csFitter;
      try {
        llTree = csFitter.fit(&mc, llTree);
      } catch (const std::exception&) {
        return result;
      }
      if (!llTree.get() || llTree->isEmpty()) return result;
    }

    const reco::TransientTrack kaonTTpre = theTTBuilder_->build(kaon.bestTrack());
    KinematicParticleFactoryFromTransientTrack preFactory;
    std::vector<RefCountedKinematicParticle> preParticles;
    preParticles.reserve(2);
    llTree->movePointerToTheTop();
    preParticles.push_back(llTree->currentParticle());
    float preChi = 0.f, preNdf = 0.f, preKaonMassErr = KaonMassErr_;
    preParticles.push_back(preFactory.particle(kaonTTpre, KaonMass_, preChi, preNdf, preKaonMassErr));

    RefCountedKinematicTree preTree;
    try {
      KinematicParticleVertexFitter preFitter;
      preTree = preFitter.fit(preParticles);
    } catch (const std::exception&) {
      return result;
    }
    if (!preTree.get() || preTree->isEmpty()) return result;

    KinematicFitResult preResult;
    preResult.set_tree(preTree);
    if (!preResult.valid() || preResult.vtxProb() < 0.001f) return result;
  }
...
KinematicFitResult
DileptonPlusXProducer::fitBToJpsiKs(const bmm::Candidate& lepton1,
				    const bmm::Candidate& lepton2,
				    const std::unique_ptr<FitCandidate>& ksCand,
				    bool jpsi_mass_constraint,
				    bool ks_mass_constraint)
{
  KinematicFitResult result;

  if (!lepton1.track() || !lepton2.track()) return result;
  if (!ksCand) return result;

  auto ksTree = ksCand->second.tree();
  if (!ksTree.get() || ksTree->isEmpty()) return result;

  if (ks_mass_constraint) {
    ParticleMass mass = KsMass_;
    MassKinematicConstraint ks_mc(mass, 1e-4f);
    KinematicParticleFitter csFitter;
    try {
      ksTree = csFitter.fit(&ks_mc, ksTree);
    } catch (const std::exception&) {
      return result;
    }
  }
  ...
  ksTree->movePointerToTheTop();
  particles.push_back(ksTree->currentParticle());

  RefCountedKinematicTree vertexFitTree;
  try {
    if (jpsi_mass_constraint) {
      ParticleMass pMass = JPsiMass_;
      TwoTrackMassKinematicConstraint jpsiConstraint(pMass);
      KinematicConstrainedVertexFitter fitter;
      vertexFitTree = fitter.fit(particles, &jpsiConstraint);
    } else {
      KinematicParticleVertexFitter fitter;
      vertexFitTree = fitter.fit(particles);
    }
...
KinematicFitResult
DileptonPlusXProducer::fitBToLLhh( const bmm::Candidate& lepton1,
				   const bmm::Candidate& lepton2,
				   const pat::PackedCandidate& had1,
				   const pat::PackedCandidate& had2,
				   ...
				   float hh_mass_constraint)
{
  ...
  // Pre-fit: mumu composite + hh composite in a 2-particle vertex
  // fit. Mass constraints, if requested, are applied to the
  // respective composite before the combination. The ultimate 4-track
  // fit runs only if this easier fit converges with vtxProb > 0.001.
  {
    auto llFit = vertexLeptonsWithKinematicFitter(lepton1, lepton2);
    if (!llFit.valid()) return result;
    auto llTree = llFit.tree();

    auto hhFit = vertexCandsWithKinematicFitter(had1, had2);
    if (!hhFit.valid()) return result;
    auto hhTree = hhFit.tree();

    KinematicParticleFitter csFitter;

    if (ll_mass_constraint > 0.f) {
      ParticleMass pMass = ll_mass_constraint;
      MassKinematicConstraint mc(pMass, JPsiMassErr_);
      try {
        llTree = csFitter.fit(&mc, llTree);
      } catch (const std::exception&) {
    ...
    if (hh_mass_constraint > 0.f) {
      ParticleMass pMass = hh_mass_constraint;
      MassKinematicConstraint mc(pMass, 1e-4f);
      try {
        hhTree = csFitter.fit(&mc, hhTree);
      } catch (const std::exception&) {
        return result;
      }
      if (!hhTree.get() || hhTree->isEmpty()) return result;
    }
    ...
    llTree->movePointerToTheTop();
    preParticles.push_back(llTree->currentParticle());
    hhTree->movePointerToTheTop();
    preParticles.push_back(hhTree->currentParticle());
