...
def train(x_train, x_test, y_train, y_test, model_name="test"):
    ...
    json.dump(feature_names,
              open("%s/%s.features" % (output_path, model_name), "w"))
    json.dump(param,
              open("%s/%s.params" % (output_path, model_name), "w"))
...
def cross_validate(x_train, y_train, model_name="test"):
    ...
    cv_results = xgb.cv(
        ...
        dtrain,
        num_round,
        seed=42,  # fix seed to get comparable scores for different parameters
...
if __name__ == "__main__":
    ...
    #    get_train_and_test_datasets_split_randomly(data)
    event_index = 0
